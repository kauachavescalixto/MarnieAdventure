...
public class AnimalPanel extends JPanel {
	...
	public AnimalPanel(int w, int h, int index, boolean lock) {
		...
		unlockBt.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				if (GamePanel.money.getMoney() > unlockprecos[index]) {
					...
					System.out.println(GamePanel.money.getMoney());
					removeAll();
					init(index);
					repaint();
				} else {
	...
	public void init(int index) {
		...
		comprarBt.setBorder(BorderFactory.createBevelBorder(1, new Color(0, 0, 0), new Color(255, 255, 255)));
	...
	public void Timer(int max, int index) {
		...
		t = new Timer(1000, new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				time.setText(text.format("%02d:%02d", TimeUnit.MILLISECONDS.toMinutes(milis - start),
						TimeUnit.MILLISECONDS.toSeconds(milis - start)
								- TimeUnit.MINUTES.toSeconds(TimeUnit.MILLISECONDS.toMinutes(milis - start))));
				if ((milis - start) / 1000 >= max) {
					progresso = 0;
					start = System.currentTimeMillis();
					milis = 0;
					count++;
